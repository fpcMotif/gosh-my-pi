...
install_via_bun() {
    ...
    echo ""
    echo "✓ Installed gmp via bun"
    echo "Run 'gmp' to get started!"
}
...
install_binary() {
    ...
    curl -fsSL "$BINARY_URL" -o "${INSTALL_DIR}/gmp"
    chmod +x "${INSTALL_DIR}/gmp"
    downloaded_native=0
    if [ "$ARCH" = "x64" ]; then
    ...
    echo ""
    echo "✓ Installed gmp to ${INSTALL_DIR}/gmp"
    echo "✓ Installed ${downloaded_native} native addon file(s) to ${INSTALL_DIR}"

    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'gmp' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'gmp'" ;;
    esac
}
