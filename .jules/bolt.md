...
## 2024-05-18 - Fast Partial Index on SQLite
**Learning:** In SQLite, for queries that filter on a specific condition and order by a column (e.g., WHERE status = 'error' ORDER BY timestamp DESC), using a partial index covering the ordered column and filtered by the condition significantly speeds up execution compared to standard indices by eliminating full index scans.
**Action:** Add partial covering index on messages(timestamp DESC) where stop_reason = 'error' in packages/stats/src/db.ts to speed up getRecentErrors.
