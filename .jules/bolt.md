...
## 2024-05-18 - Missing composite indices causing B-TREE usage
**Learning:** In `bun:sqlite`, missing indices on commonly queried fields (like combining filtering with `stop_reason` and sorting with `timestamp`) cause full table scans or temporary B-TREE usage that block the Node.js event loop and degrade application responsiveness.
