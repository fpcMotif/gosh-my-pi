...
## 2024-05-19 - React Chart.js Table Rendering Performance
**Learning:** In `react-chartjs-2`, creating inline `chartData` or `options` objects inside list/table rows causes severe performance degradation due to React re-rendering the chart on every parent state change (like expanding a row). `Line` or `Bar` components do full re-renders if references change.
