...
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop. Adding an `onClick` handler that checks if `e.target === dialogRef.current` allows users to click outside the modal content to close it, which is an expected UX pattern for modals.
**Action:** Always add an `onClick` handler to native `<dialog>` elements to support backdrop dismissal when appropriate.
