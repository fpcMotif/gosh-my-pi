...
## 2024-07-18 - [Native Dialog Backdrop Dismissal]
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking their `::backdrop`. They require a specific `onClick` handler on the dialog itself that checks `if (e.target === dialogRef.current)` to determine if the click was on the backdrop rather than the dialog's contents.
**Action:** Whenever using a native `<dialog>` element for a modal, always remember to add an `onClick` listener to check the event target and invoke the close mechanism to ensure standard backdrop dismissal UX.
