...
## 2026-08-28 - Click-outside to Close Native Dialog
**Learning:** Native HTML `<dialog>` elements show a backdrop via `showModal()`, but clicking the backdrop does not close the dialog by default, frustrating users who expect standard modal behavior.
**Action:** Attach an `onClick` handler directly to the `<dialog>` element and check `if (e.target === dialogRef.current) onClose();` to ensure only the backdrop click triggers the close, preventing clicks inside the dialog content from bubbling up and closing it.
