...
## 2026-09-02 - ARIA Tabs Accessibility
**Learning:** When implementing ARIA tab navigation, always explicitly link `role="tab"` buttons to their corresponding `role="tabpanel"` containers using `aria-controls` and `id` on the tab, and `aria-labelledby` and `id` on the panel for screen reader accessibility.
**Action:** Add ARIA linking attributes to all tab components.
