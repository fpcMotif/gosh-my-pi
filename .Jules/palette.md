...
## 2024-05-12 - [Accessible Interactive Table Rows]
**Learning:** When making `<tr>` elements interactive via `onClick` in this app, they are inaccessible to keyboard and screen reader users by default.
**Action:** Always add `role="button"`, `tabIndex={0}`, an `onKeyDown` handler (listening for `Enter` and `Space`), and explicit `focus-visible` styling to ensure parity with mouse interactions.
