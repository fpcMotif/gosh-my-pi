...
## 2026-08-27 - Click-Outside Dismiss Pattern
**Learning:** Implementing click-outside-to-close behavior on native HTML `<dialog>` elements is crucial for intuitive UX, but must be implemented carefully to avoid false positives from clicks inside the modal content.
**Action:** Add an `onClick` handler directly to the `<dialog>` element and strictly verify that the event target matches the dialog element itself (`e.target === dialogRef.current`) before executing the close callback, ensuring only clicks on the actual backdrop are registered.
