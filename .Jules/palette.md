...
## 2024-05-18 - Native Dialog Backdrop Dismissal
**Learning:** Native HTML `<dialog>` elements do not automatically close when the user clicks on the backdrop outside the dialog content. This can lead to a frustrating user experience where they expect a click outside to dismiss the modal, but nothing happens.
**Action:** When implementing a `<dialog>` element, add an `onClick` handler that checks if `e.target === dialogRef.current` and calls the dismissal function to enable backdrop dismissals.
