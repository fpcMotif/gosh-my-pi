...
## 2024-05-20 - Click-Outside to Close Native Dialog
**Learning:** The native HTML <dialog> element handles Escape key dismissal automatically when opened via showModal(), but it does not inherently close when a user clicks the backdrop. This creates an inconsistent UX for modals.
**Action:** Always implement an onClick handler on <dialog> elements to verify if the event target matches the dialog reference, allowing backdrop clicks to close the modal.
