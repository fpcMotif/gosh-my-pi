...
## 2024-05-16 - Add backdrop click dismissal to native <dialog>

**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop (the area outside the modal itself). Relying solely on the close button can lead to poor usability, as users naturally expect clicking outside a modal will close it.
**Action:** When using `<dialog>` in React components, always add an `onClick` handler that checks if `e.target === dialogRef.current` and calls the close function to enable backdrop dismissals.
