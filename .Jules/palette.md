...
## 2024-07-30 - [Dismiss Dialog via Backdrop Click]
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop, leading to poor UX where users must explicitly click the close button.
**Action:** Always add an onClick handler to `<dialog>` components that checks `if (e.target === dialogRef.current)` and calls the close function.
