...
## 2026-08-23 - Native Dialog Backdrop Click
**Learning:** Native HTML <dialog> elements do not automatically close when clicking their backdrop (outside the dialog). Attaching an onClick handler that checks if the event target is the dialog element itself is a simple and effective way to implement click-outside-to-close behavior.
**Action:** Always add an onClick handler to native <dialog> elements to support dismissing the modal by clicking the backdrop.
