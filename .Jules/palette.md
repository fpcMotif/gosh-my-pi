...
## 2024-05-20 - Add Backdrop Click Dismissal for Native Dialogs

**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop, unlike many third-party modal libraries. This breaks user expectations for standard modal interactions, negatively impacting intuitive UX.
**Action:** When using the native `<dialog>` element in React components, always add an `onClick` handler that checks if `e.target === dialogRef.current` and calls the close function to enable expected backdrop dismissals.
