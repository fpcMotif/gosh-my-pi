...
## 2024-05-20 - Add backdrop click dismissal to native dialogs

**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop. This can confuse users who expect to be able to click outside the modal to dismiss it, a common pattern in web interfaces.
**Action:** Always add an `onClick` handler to `<dialog>` components that checks if `e.target === dialogRef.current` and calls the close function to enable backdrop dismissals.
