...
## 2024-07-23 - Dismiss Native Dialog on Backdrop Click
**Learning:** Native HTML `<dialog>` elements do not automatically close when a user clicks on the backdrop (the area outside the dialog). This violates common UX expectations where backdrop clicks typically dismiss modals.
**Action:** When using `<dialog>` in React components, add an `onClick` handler that checks if `e.target === dialogRef.current` and calls the close function to enable intuitive backdrop dismissals.
