...
## 2024-06-01 - Add backdrop click dismissal to native `<dialog>` elements
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop. Adding an `onClick` handler that checks if `e.target === dialogRef.current` enables intuitive backdrop dismissal.
**Action:** Always add backdrop click dismissal to `<dialog>` elements by checking if the click target matches the dialog ref.
