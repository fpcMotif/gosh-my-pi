...
## 2024-07-16 - Accessible Interactive Table Rows & Empty States
**Learning:** Making non-interactive elements like `<tr>` clickable shouldn't use `role="button"` as it breaks native screen reader table semantics. Empty states ("No requests found") feel much more polished when accompanied by a muted icon (like `Inbox`) rather than just plain text.
**Action:** Use `tabIndex={0}`, an `onKeyDown` handler (for Enter/Space), and `aria-label`/`title` tooltips for interactive table rows instead of `role="button"`. Enhance empty state text with an appropriate descriptive icon in a flex column layout.
