...
**Learning:** Native HTML `<dialog>` elements do not automatically close when the user clicks the backdrop. This breaks user expectations for modal dismissals.
**Action:** When using `<dialog>` in React, always add an `onClick` handler that checks if `e.target === dialogRef.current` and calls the close function to enable backdrop dismissals.
