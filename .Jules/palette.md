...
## 2024-05-16 - Native Dialog Backdrop Dismissal
**Learning:** Native HTML `<dialog>` elements do not automatically close when the user clicks on the backdrop (the area outside the dialog content). This can lead to a frustrating user experience where the only way to close the modal is by finding and clicking the close button or pressing Escape.
**Action:** When using the native `<dialog>` element, always add an `onClick` handler that checks if `e.target === dialogRef.current` and calls the close function. This enables intuitive backdrop dismissals.
