...
## 2024-08-06 - Native Dialog Backdrop Click
**Learning:** The native `<dialog>` element backdrop can be clicked, but it does not close the dialog by default. Clicking the backdrop fires an `onClick` event with `e.target` set to the `<dialog>` itself.
**Action:** Always add an `onClick` handler to `<dialog>` elements checking `e.target === e.currentTarget` to implement dismiss-on-backdrop-click behavior.
