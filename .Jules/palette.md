...
## 2024-05-20 - Add click-outside-to-close pattern for Dialog Modals
**Learning:** Native HTML `<dialog>` elements used as modals do not close when the backdrop is clicked by default, which breaks user expectations for modal interactions.
**Action:** Always implement a click-outside-to-close pattern for `<dialog>` elements by attaching an `onClick` handler that checks `if (e.target === dialogRef.current)` to ensure only the backdrop triggers the close action.
