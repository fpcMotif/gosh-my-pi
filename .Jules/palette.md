...
## 2024-05-18 - Click-Outside to Close Native Dialogs
**Learning:** Native HTML `<dialog>` elements do not automatically close when their backdrop is clicked. Attaching an `onClick` listener to the dialog and verifying `e.target === dialogRef.current` is a straightforward and reliable way to implement this expected UX interaction without interfering with the dialog's inner contents.
**Action:** Always implement click-outside-to-close on native `<dialog>` elements using `onClick={(e) => { if (e.target === dialogRef.current) onClose(); }}` to provide a standard and expected UX interaction.
