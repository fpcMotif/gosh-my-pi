...
## 2024-07-26 - Native HTML `<dialog>` Backdrop Dismissal
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop, unlike some custom modal implementations. This can trap users who expect to be able to click outside the modal to dismiss it, leading to a frustrating UX.
**Action:** When using `<dialog>` in React components, add an `onClick` handler to the dialog element that checks if `e.target === dialogRef.current` and calls the close function to enable backdrop dismissals.
