...
## 2026-09-06 - Native Dialog Click-Outside-To-Close
**Learning:** Native `<dialog>` elements in React don't automatically close when clicking the backdrop.
**Action:** Attach an `onClick` handler directly to the dialog that checks if `e.target === dialogRef.current` to implement intuitive "click-outside-to-close" behavior.
