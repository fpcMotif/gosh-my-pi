...
## 2024-05-18 - Click Outside to Close Dialog
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop (outside the dialog). Providing a click-outside-to-close behavior greatly enhances UX, especially for modals used as side-panels.
**Action:** Attach an `onClick` handler to the `<dialog>` and check `if (e.target === dialogRef.current)` to conditionally close the modal when the backdrop is clicked.
