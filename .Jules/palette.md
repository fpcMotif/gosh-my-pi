...
## 2024-06-21 - Make Table Rows Keyboard Accessible
...
**Action:** Ensure keyboard accessibility by only adding `tabIndex={0}`, an `onKeyDown` handler for 'Enter' and 'Space' (calling `e.preventDefault()` to stop page scrolling on Space), an `aria-label`, and `focus-visible` styles using existing Tailwind utility classes.
