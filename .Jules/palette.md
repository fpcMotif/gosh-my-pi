...
## 2024-07-22 - Add Backdrop Click to Dismiss Dialog

**Learning:** Native HTML `<dialog>` elements do not automatically close when the user clicks the backdrop. This creates a frustrating UX trap if the user attempts to click away from the modal to dismiss it, as is common in most modern web patterns.
**Action:** When using `<dialog>` in React components, always attach an `onClick` handler to the `<dialog>` that checks `if (e.target === dialogRef.current)` and calls the close/dismiss function to ensure the backdrop is dismissible.
