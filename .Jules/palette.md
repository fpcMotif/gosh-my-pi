...
## 2024-07-31 - Native Dialog Backdrop Click Dismissal
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop, which is a common expectation for mouse users. This makes modals unintuitive to close.
**Action:** When using `<dialog>` in React components, add an `onClick` handler that checks if `e.target === dialogRef.current` and calls the close function to enable backdrop dismissals.
