...
## 2026-08-29 - Click-outside to close for native HTML dialog
**Learning:** When using a native HTML <dialog> element in React, you can implement a "click outside to close" pattern without an external backdrop element by checking if the click target is the dialog itself.
**Action:** Attach an onClick handler directly to the <dialog> element and check if e.target === dialogRef.current to close it intuitively.
