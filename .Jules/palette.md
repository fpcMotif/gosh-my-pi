...
## 2024-05-24 - Add backdrop click dismissal to native dialogs
**Learning:** Native HTML `<dialog>` elements do not automatically close when clicking the backdrop, which breaks user expectations for modal interactions.
**Action:** Always add an `onClick` handler to `<dialog>` elements that checks if `e.target === dialogRef.current` and calls the close function to enable backdrop dismissals.
