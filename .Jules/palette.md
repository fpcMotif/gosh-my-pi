...
**Learning:** Native HTML `<dialog>` elements do not automatically close when their backdrop is clicked. This can trap users who intuitively click outside the modal content expecting it to close.
**Action:** When using `<dialog>` in React, always add an `onClick` handler that checks `e.target === dialogRef.current` to call the close function, enabling smooth backdrop dismissals.
