...
## 2025-06-14 - Interactive Table Rows Accessibility

**Learning:** When making non-interactive semantic HTML elements (like `<tr>`) clickable in React, adding `role="button"` breaks native screen reader table semantics.
**Action:** Ensure keyboard accessibility by adding `tabIndex={0}`, an `onKeyDown` handler for 'Enter' and 'Space', an `aria-label`, and `focus-visible` styles, without altering the element's semantic role.
