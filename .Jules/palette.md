...
## 2024-08-24 - [Click Outside to Close Native Dialogs]
**Learning:** Native HTML <dialog> elements can be easily closed when clicking their backdrop by attaching an onClick handler and checking if the event target matches the dialog element, ensuring inner content clicks are ignored.
**Action:** Use onClick={(e) => e.target === dialogRef.current && onClose()} on native <dialog> elements to support click-outside-to-close UX.
