...
## 2024-05-18 - [ARIA Tab Navigation Linkage]
**Learning:** Screen readers need explicit links between tab buttons and their content panels.
**Action:** Always link `role="tab"` buttons to their `role="tabpanel"` containers using `aria-controls` on the tab and `aria-labelledby` on the panel with matching IDs.
