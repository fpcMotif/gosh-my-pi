...
## 2024-05-20 - Connect ARIA Tabs to Tabpanels
**Learning:** The `role="tablist"`, `role="tab"`, and `role="tabpanel"` roles work best when explicitly linked. While focus management covers keyboard access, screen readers rely on explicit connections mapping each tab (`aria-controls`) to its corresponding panel, and panels back to their tabs (`aria-labelledby`).
**Action:** Always link `role="tab"` buttons to their corresponding `role="tabpanel"` containers using `aria-controls`, `id`, and `aria-labelledby`.
