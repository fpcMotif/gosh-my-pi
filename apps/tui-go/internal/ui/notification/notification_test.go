...
// TestOSCBackend_DarwinIconString verifies that a string icon value (as used on
// darwin, where notification.Icon is "") never produces an OSC 99 icon payload.
func TestOSCBackend_DarwinIconString(t *testing.T) {
	t.Parallel()

	backend := notification.NewOSCBackend("", true)
	s := extractRawString(t, backend.Send(notification.Notification{
		Title:   "Test",
		Message: "No icon on darwin",
	}))
...
func TestDetectOSC99Support_ValidResponse(t *testing.T) {
	...
	seq := "\x1b]99;i=gmp-osc99-query:p=?;p=title\x07"
...
func TestDetectOSC99Support_MultipleCapabilities(t *testing.T) {
	...
	seq := "\x1b]99;i=gmp-osc99-query:p=?;p=title,body,icon\x07"
	require.True(t, notification.DetectOSC99Support(seq))
}
...
func TestDetectOSC99Support_InvalidCommand(t *testing.T) {
	...
	seq := "\x1b]98;i=gmp-osc99-query:p=?;p=title\x07"
...
func TestDetectOSC99Support_NoQueryFlag(t *testing.T) {
	...
	seq := "\x1b]99;i=gmp-osc99-query;p=title\x07"
	require.False(t, notification.DetectOSC99Support(seq))
}
...
func TestDetectOSC99Support_NoTitleCapability(t *testing.T) {
	...
	seq := "\x1b]99;i=gmp-osc99-query:p=?;p=body\x07"
	require.False(t, notification.DetectOSC99Support(seq))
}
...
func TestOSC99QuerySequence(t *testing.T) {
	...
	require.Contains(t, seq, "i=gmp-osc99-query")
