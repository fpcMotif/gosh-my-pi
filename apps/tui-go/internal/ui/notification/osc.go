...
const osc99QueryID = "gmp-osc99-query"

// DetectOSC99Support parses an OSC response sequence and returns true if it
// indicates OSC 99 notification support. This should be called from the
...
// OSC99QuerySequence returns the OSC 99 query sequence used to detect terminal
// support. This should be sent during capability detection.
...
// NewOSCBackend creates a new OSC notification backend. If supports99 is true,
// it uses OSC 99; otherwise it falls back to OSC 777.
func NewOSCBackend(icon any, supports99 bool) *OSCBackend {
	iconBytes, _ := icon.([]byte)
	return &OSCBackend{
		icon:       iconBytes,
...
func (b *OSCBackend) sendOSC99(n Notification) tea.Cmd {
	...
	b.notifySeq++
	id := fmt.Sprintf("gmp-%d", b.notifySeq)

	appName := "gmp"
	notificationType := "gmp-notification"
