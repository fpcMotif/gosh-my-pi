...
// document (gap G5: the assistant renderMarkdown previously did a
// full glamour render of the entire accumulated body on every
// text_delta — O(messageLength) per flush).
//
...
// concurrent invocation on a shared renderer. The TUI is
...
//
// We render the prefix separately rather than try to recover it
// from the full render output because two renders concatenated ≠ a
// single render of the whole, and we prefer the cached prefix
// render to be byte-for-byte what we'd produce on a future cached
// call.
...
//     would change once the underline arrived.
//
...
// uses the SIMPLEST viable conservative rule:
//
