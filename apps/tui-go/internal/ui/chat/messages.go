...
type highlightableMessageItem struct {
	...
	// bumps it on every observable change so the G6 list memo and
...
// version so the G6 list-level memo invalidates frozen entries on the
// next render.
...
type cachedMessageItem struct {
	...
	// or selection prefix to every line). Items rebuild this every frame
	// today; caching it keyed by (prefixedWidth, prefixedKey) turns
	// Render into a pointer return when item state is stable.
	//
	// Invalidation lives in clearCache; callers must additionally bypass
	// this cache whenever the prefixed output would not be stable
	// (spinner ticks, active highlight ranges) by not calling
...
// per-line prefix (focused/blurred, section fingerprint, ...).
...
type focusableMessageItem struct {
	...
	// bumps it whenever focus actually flips so the G6 list memo
...
// Finished implements list.Item. AssistantInfoItems are emitted once
// the assistant turn completes and never change afterward, so they are
// always safe to freeze.
