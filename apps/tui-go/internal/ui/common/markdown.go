...
import (
	...
	"github.com/fpcMotif/gosh-my-pi/apps/tui-go/internal/ui/xchroma"
)
...
// API). The TUI is single-threaded so production never contends,
// but parallel callers (most notably parallel tests) must
// serialize via [LockMarkdownRenderer]. Treat the renderer as
// effectively pinned to one goroutine at a time.
...
// whenever the active styles change so subsequent renderers pick
// up the new ansi.StyleConfig.
//
...
// mutexes by pointer so the lock granularity matches the renderer
// cache granularity (one mutex per (width, palette) renderer
// instance, not one mutex for the entire cache).
//
// Lock ordering: when both mdCacheMu and rendererLocksMu are
// needed (only in InvalidateMarkdownRendererCache), acquire
...
// logical operation should hold the mutex for the entire sequence
// so other goroutines do not interleave their own Render calls and
// corrupt the renderer state. The streaming-markdown stable-prefix
// cache is the immediate consumer; other call sites that today
// issue exactly one Render call per item render are safe without
// locking under the single-threaded TUI Update loop, but should
// adopt this lock if they ever run in parallel.
