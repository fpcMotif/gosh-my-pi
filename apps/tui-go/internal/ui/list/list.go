...
type List struct {
	...
	// cache is the G6 list-level render memo, keyed by item pointer.
	// Each entry stores the rendered content, a pre-split slice of
	// lines (so the various traversals share one render per frame),
	// the height, and the keys that govern invalidation (width and
	// version). The frozen flag mirrors the freeze rule: once a
	// Finished() item is rendered, subsequent draws return the stored
	// output verbatim without calling back into Render.
	cache map[Item]*listCacheEntry

	// freezeSuppressed marks items the list must not freeze on the
	// next render even when their Finished() reports true. This is the
	// selection-drag escape hatch: items inside an active selection
	// range render as live items so that per-line highlight overlays
	// land on the latest content. Cleared on EndSelectionDrag.
...
// The result is served from the G6 cache when possible — see
