(function () {
	...
	function renderLsp(name, args, result, ctx) {
		...
		if (args.query)
			head += ` <span class="tool-arg-key">query=</span><span class="tool-arg-val">${escapeHtml(String(args.query))}</span>`;
		if (args.new_name)
			head += ` <span class="tool-arg-key">→</span> <span class="tool-arg-val">${escapeHtml(String(args.new_name))}</span>`;
		let html = `<div class="tool-header">${head}</div>`;
		if (result) {
			const output = ctx.getResultText();
	...
	function renderTodoWrite(name, args, result, ctx) {
		...
		if (ops) {
			...
			for (const op of ops) {
				...
				if (op?.task && typeof op.task === "object" && op.task.content)
					line += ` ${escapeHtml(truncate(String(op.task.content), 80))}`;
			...
			for (var __i = 0; __i < phases.length; __i++) {
				...
				if (Array.isArray(phase.tasks)) {
					for (const task of phase.tasks) {
						...
						const icon =
							status === "completed" ? "✓" : status === "in_progress" ? "→" : status === "abandoned" ? "✕" : "○";
	...
	function renderJob(name, args, result, ctx) {
		const badges = [];
		const pollIds = Array.isArray(args.poll)
			? args.poll
