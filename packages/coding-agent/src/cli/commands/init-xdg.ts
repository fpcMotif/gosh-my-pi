...
export async function initXdg(): Promise<void> {
	...
	const dirs = [
		path.join(dataHome, STORAGE_APP_NAME),
		path.join(stateHome, STORAGE_APP_NAME),
		path.join(cacheHome, STORAGE_APP_NAME),
	];
	...
	console.log("are set in your shell profile for gmp to use them.");
}
