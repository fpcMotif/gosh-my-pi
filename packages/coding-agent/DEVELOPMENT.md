...
- Input: JSON lines from `readLines(Bun.stdin.stream())`, parsed one at a time.
...
- State: `get_state`, `set_todos`, `set_host_tools`
...
`auth.login` accepts an optional provider. Without one, the backend asks the
host to pick a provider through `auth.pick_provider`. The provider commands
list supported providers and providers with stored credentials.
