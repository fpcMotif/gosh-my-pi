...
/**
 * Linear back-off retry policy for Copilot model errors. Composes
 * `basicRetryPolicy` (recurs 2) with delays 400 ms, 800 ms
...
/**
 * Wrap an Effect with the Copilot retry Schedule when (and only when) the
 * provider is `github-copilot`. Other providers pass through unchanged.
 */
