...
export function CostSummary({ costSeries }: CostSummaryProps) {
	// ⚡ Bolt Optimization:
	// Moved Date.now() inside useMemo to prevent the fluctuating 'cutoff' value
	// from breaking memoization on every render, avoiding unnecessary array filtering.
	...
	const previous = useMemo(() => {
		...
		return costSeries.filter(p => p.timestamp >= prevCutoff && p.timestamp < cutoff);
	}, [costSeries]);
