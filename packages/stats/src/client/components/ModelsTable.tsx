...
const EMPTY_TREND_DATA = [] as never[];
...
// ⚡ Bolt: Memoize components that render inside table lists to prevent O(n) re-renders
const TrendChart = memo(function TrendChart({
	...
	// ⚡ Bolt: Memoize chartData to ensure stable object references
...
const PerformanceChart = memo(function PerformanceChart({
	...
	// ⚡ Bolt: Memoize chartData and options to ensure stable object references
	const chartData = useMemo(
		() => ({
			labels: data.map(d => format(new Date(d.timestamp), "MMM d")),
			datasets: [
