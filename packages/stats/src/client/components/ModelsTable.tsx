...
const TrendChart = memo(function TrendChart({
	...
	// ⚡ Bolt: Memoize chartData to prevent unnecessary re-renders of the chart component
...
const PerformanceChart = memo(function PerformanceChart({
	...
	// ⚡ Bolt: Memoize chartData to prevent unnecessary re-renders
	const chartData = useMemo(
		() => ({
			labels: data.map(d => format(new Date(d.timestamp), "MMM d")),
			datasets: [
	...
	// ⚡ Bolt: Memoize options to prevent unnecessary re-renders
