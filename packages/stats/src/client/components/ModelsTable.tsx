...
const TrendChart = memo(function TrendChart({
	...
	// ⚡ Bolt: Memoize chartData to enforce stable object references and prevent re-rendering the chart on every parent state change.
...
const PerformanceChart = memo(function PerformanceChart({
	...
	// ⚡ Bolt: Memoize chartData and options to enforce stable object references.
	const chartData = useMemo(
		() => ({
			labels: data.map(d => format(new Date(d.timestamp), "MMM d")),
			datasets: [
