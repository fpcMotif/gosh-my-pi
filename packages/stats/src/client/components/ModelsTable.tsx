...
const TrendChart = memo(function TrendChart({
	...
	// ⚡ Bolt: Memoize chartData to prevent unnecessary re-renders of the chart.
...
const PerformanceChart = memo(function PerformanceChart({
	...
	// ⚡ Bolt: Memoize chartData and options to maintain stable object references and avoid expensive re-renders in react-chartjs-2.
	const chartData = useMemo(
		() => ({
			labels: data.map(d => format(new Date(d.timestamp), "MMM d")),
			datasets: [
