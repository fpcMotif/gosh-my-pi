...
const TrendChart = memo(function TrendChart({
	...
	// ⚡ Bolt: Memoize chartData and use React.memo to prevent expensive re-renders in list. Impact: Reduces chart re-renders significantly.
...
const PerformanceChart = memo(function PerformanceChart({
	...
	// ⚡ Bolt: Memoize chartData and options to prevent expensive Chart.js re-renders in list. Impact: Reduces chart re-renders significantly.
	const chartData = useMemo(
		() => ({
			labels: data.map(d => format(new Date(d.timestamp), "MMM d")),
			datasets: [
