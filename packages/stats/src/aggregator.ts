...
export function syncAllSessions(): Promise<{ processed: number; files: number }> {
	...
	// If a sync is already in progress (e.g. triggered by concurrent API requests),
	// return the existing promise rather than starting a redundant disk read & DB write operation.
	if (syncPromise) {
		return syncPromise;
	}
