...
export async function parseSessionFile(
	...
	// Performance Optimization: Use Bun.file(path).slice(start).bytes() instead of loading
	// the entire file into memory and then using .subarray().
	// We first check the file size to avoid out-of-bounds slice offsets.
	const file = Bun.file(sessionPath);
	const size = file.size;

	if (size === 0 && fromOffset > 0) {
		// File might have been truncated or not exist yet
