...
export async function parseSessionFile(
	...
	try {
		...
		const size = file.size; // Get total file size first
		start = Math.max(0, Math.min(fromOffset, size));

		if (start >= size) {
			unprocessed = new Uint8Array(0); // Nothing new to read
		} else {
			// Bolt: Performance Optimization
			// Only read the appended bytes into memory using slice() instead of
			// loading the entire file and subarraying. This drastically reduces
			// memory allocations and GC pressure for large session files.
