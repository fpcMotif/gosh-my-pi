...
export async function parseSessionFile(
	...
	const size = file.size; // Returns 0 if file does not exist, avoids Enoent here
	const start = Math.max(0, Math.min(fromOffset, size));

	let unprocessed: Uint8Array;
	try {
		// Performance optimization: Avoid loading the entire file into memory by using slice().bytes()
		// instead of loading the full file and calling .subarray(start). This significantly reduces
		// memory usage and I/O time when parsing appended data in large session files.
