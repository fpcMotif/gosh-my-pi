...
export async function parseSessionFile(
	...
	// Optimize reading by slicing large session files correctly instead of subarray on entire bytes array.
	// We read size first, and handle ENOENT gracefully if file doesn't exist.
	const size = file.size; // returns 0 cleanly if file doesn't exist
	if (size === 0) return { stats: [], newOffset: fromOffset };
