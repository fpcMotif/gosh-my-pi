...
export async function parseSessionFile(
	...
	// ⚡ Bolt: Optimize reading appended JSONL data by slicing directly from the file.
	// This avoids loading the entire file into memory (which was an O(N) memory overhead)
	// and instead only reads the newly appended bytes, saving significant memory and CPU.
	const file = Bun.file(sessionPath);
	const start = Math.max(0, Math.min(fromOffset, file.size));
