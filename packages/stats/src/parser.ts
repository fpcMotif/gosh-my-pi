...
export async function parseSessionFile(
	...
	let start: number;
	try {
		// Performance optimization: Avoid reading the entire file into memory just to discard
		// the previously processed portion. Read only the new bytes starting from fromOffset.
		const file = Bun.file(sessionPath);
		const size = file.size; // Returns 0 if file doesn't exist, which is handled gracefully
